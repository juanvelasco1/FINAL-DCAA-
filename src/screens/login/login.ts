...
class Login extends HTMLElement {
    ...
    connectedCallback() {
     ...
     handleLoginButton() {
        ...
        if (this.validateUser(res, credentials.email, credentials.password)) {
            ...
            dispatch(redirect(Screens.HOME));
        } else {
    ...
    render() {
        ...
        signupText.innerHTML = "You do not have an account? <button id='signup-button' class='signup-link'>sign up here</button>";
        ...
        aSignupBtn?.addEventListener("click", () => dispatch(redirect(Screens.SIGNUP)));
